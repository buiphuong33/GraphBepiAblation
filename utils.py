...
class chain:
    ...
    def load_dssp(self, path):
        ...
        # ----- Ablation option: zero-out DSSP features -----
        # For DSSP ablation experiments we remove DSSP information by
        # replacing the per-residue DSSP vector with zeros. This lets
        # the GNN be trained without DSSP while preserving the
        # expected tensor shapes in the rest of the pipeline.
        try:
            self.dssp = torch.zeros_like(self.dssp)
        except Exception:
            # fallback: if dssp is not a tensor or shape mismatch, ignore
            pass
...
def export_tabular(root, out_dir="./tabular", split='all'):
    ...
    for s in tqdm(samples, desc='Exporting residues'):
        ...
        for i in range(L):
            ...
            # Remove DSSP from tabular features for XGBoost ablation.
            # Exported features: ESM embeddings + degree + neighbor-edge-mean + amino_id
            x = np.concatenate([
                esm_i.astype(np.float32),
                np.array([deg], dtype=np.float32),
                neigh_edge_mean.astype(np.float32),
                np.array([amino_id], dtype=np.float32)
            ])
            rows_X.append(x)
